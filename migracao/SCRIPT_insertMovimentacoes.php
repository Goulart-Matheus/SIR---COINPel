...
$query_cooc->exec("SELECT * FROM canil.tutor_movimentacoes");
...
foreach ($query_cooc->record as $user) 
{
    ...
    $query_sir->exec("INSERT INTO canil.tutor_movimentacoes(
	id_tutor_movimentacoes, id_tutor, observacao, tipo, login, ip, dt_alteracao, hr_alteracao)
	VALUES ('{$user['id_tutor_movimentacoes']}', '{$user['id_tutor']}', '$obs', '{$user['tipo']}', '$login ', '$ip', '{$user['dt_alteracao']}', '{$user['hr_alteracao']}');");
    echo $query_sir->sql;
    echo "<br><br>";    
