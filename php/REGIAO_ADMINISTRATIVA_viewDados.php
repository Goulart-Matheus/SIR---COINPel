...
if (isset($filter)) {
    $condicao .= $form_habilitado != "" ? " AND habilitado='" . $form_habilitado . "'" : "";
}

$query->exec("SELECT id_regiao_administrativa , nome , habilitado 
                    FROM denuncias.regiao_administrativa
...
$report_subtitulo   = "Região Administrativa";
...
if ($print) {
    ...
    if (isset($remove)) {
        ...
        if (!isset($id_regiao_administrativa)) {
            ...
            for ($c = 0; $c < sizeof($id_regiao_administrativa); $c++) {

                $where = array(0 => array('id_regiao_administrativa', $id_regiao_administrativa[$c]));
                $querydel->deleteTupla('denuncias.regiao_administrativa', $where);
            }

            unset($_POST['id_regiao_administrativa']);
        }
    }
...
$tab->setTab('Região Administrativa', 'fas fa-file-code', $_SERVER['PHP_SELF']);
$tab->setTab('Nova Região Administrativa', 'fas fa-plus', 'REGIAO_ADMINISTRATIVA_form.php');

$tab->printTab($_SERVER['PHP_SELF']);

$n = $paging->query->rows();

include 'REGIAO_ADMINISTRATIVA_view.php'
?>
            ...
            <div class="card-header border-bottom-1 mb-3 bg-light-2">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-4 text-center text-md-right mt-2 mt-md-0">
                        ...
                        <button type="button" class="btn btn-sm btn-green text-light" data-toggle="modal" data-target="#REGIAO_ADMINISTRATIVA_view">
                ...
                <table class="table table-sm text-sm">
                    ...
                    <tbody>
                        ...
                        while ($n--) {
                            ...
                            $js_onclick = "OnClick=javascript:window.location=('REGIAO_ADMINISTRATIVA_edit.php?id_regiao_administrativa=" . $paging->query->record[0] . "')";

                            echo "<tr class='entered'>";

                            echo "<td valign='top'><input type=checkbox class='form-check-value' name='id_regiao_administrativa[]' value=" . $paging->query->record[0] . "></td>";
