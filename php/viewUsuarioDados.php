...
if ($form_habilitado<>"") $where=" AND habilitado='$form_habilitado'";

$query->exec("SELECT login,nome FROM usuario WHERE nome ilike '%$form_nome%' $where");
...
if ($print){
    ...
    if (isset($remove)) {
        if (!isset($login))
            ...
            for ($c=0; $c < sizeof($login); $c++) {
                $where =array(0 => array('login', $login[$c]));
                $querydel->deleteTupla('usuario', $where);
            }
            unset($_POST['login']);
        }
    }
            ...
            <div class="card p-1">
                ...
                <div class="card-body pt-0">
                    <table class="table table-striped">
                        ...
                        <tr>
                            <td width="5px"></td>
                            <td style='<?echo $sort->verifyItem(1);?>'><?echo $sort->printItem(1, $sort->sort_dir, 'Login')  ?></td>
                            <td style='<?echo $sort->verifyItem(2);?>'><?echo $sort->printItem(2, $sort->sort_dir, 'Nome') ?></td>
                        </tr>
                        <?
                        while ($n--) {
                            $paging->query->proximo();
                            $js_onclick ="OnClick=javascript:window.location=('editUsuario.php?login=" . $paging->query->record[0] . "')";

                            echo "<td valign='top'><input type=checkbox class='form-check-value' name='login[]' value=" . $paging->query->record[0] ."></td>";
